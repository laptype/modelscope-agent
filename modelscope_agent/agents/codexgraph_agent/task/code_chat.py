...
from modelscope_agent.agents.codexgraph_agent.cypher_agent import CODE_SEARCH_FORMAT
...
class CodexGraphAgentChat(CodexGraphAgentGeneral):
    ...
    def _run(self,
        ...
        for iter in range(self.max_iterations):
            ...
            extracted_analysis, _ = extract_text_between_markers(response_text, "[start_of_analysis]", "[end_of_analysis]")
            extracted_code_search, _ = extract_text_between_markers(response_text, "[start_of_code_search]", "[end_of_code_search]")
            answer_question, _ = extract_text_between_markers(response_text, "[start_of_answer]", "[end_of_answer]")

            self.update_agent_message(response_to_msg(extracted_analysis, extracted_code_search, answer_question))

            if not extracted_code_search and not answer_question:
                msg = "The text between the markers [start_of_code_search] and [end_of_code_search], as well as the text between the markers [start_of_answer] and [end_of_answer], is empty."
                messages.append({"role": "user", "content": msg})
                continue

            elif extracted_code_search:
                cypher_queries = self.cypher_queries_template.substitute(text_queries=extracted_code_search)
                user_response = self.cypher_agent.run(cypher_queries, retries=self.max_iterations_cypher)

                if not user_response:
                    msg = "Cypher Code Assistant encountered issues while processing Cypher queries. Please try writing simpler and clearer text queries, and ensure that the corresponding parameters are correct."
                    messages.append({"role": "user", "content": msg})
                else:
            ...
            if iter < self.max_iterations - 1:
                msg = (
                    ...
                    f"{ANSWER_FORMAT}"
                    "\n\nNOTE:"
                    "\n- If you have already answer the user's question, do not write any search text queries."
                )
                messages.append({"role": "user", "content": msg})
