...
def setting(page_name, path='CG_conversation'):
    ...
    st.session_state[page_name]['setting']['history_list'] = get_json_files(path)
    with st.expander("Settings"):
        ...
        with st.container():
            ...
            with col1:
                st.session_state.shared['setting']['neo4j']['user'] = st.text_input(
                    "User",
            ...
            with col3:
                st.session_state.shared['setting']['neo4j']['password'] = st.text_input(
                    "Password",
                    type="password",
                    value=st.session_state.shared['setting']['neo4j']['password']
                )

            with col4:
                st.session_state.shared['setting']['neo4j']['database_name'] = st.text_input(
                    "Database Name",
                    value=st.session_state.shared['setting']['neo4j']['database_name']
                )
            with col5:
                st.write("\n\n")
                with st.form(key='test_connection', border=False):
                    # 创建按钮
                    st.session_state[page_name]['test_connect_button'] = st.form_submit_button(label='Test Connection', use_container_width=True)
        ...
        with st.container():
            ...
            with col2:
                repo_path = st.text_input("Repo Path", placeholder="Enter repo path here",
                                          value=st.session_state.shared['setting']['repo_path'], key='repo_path_input')
                if repo_path != st.session_state.shared['setting']['repo_path']:
                    update_config('repo_path', repo_path)
            with col3:
                project_options = st.session_state.shared['setting']['project_list']
                project_id = st.selectbox("Project ID", project_options,
                                          project_options.index(st.session_state.shared['setting']['project_id']))
            ...
            with col4:
                ...
                with st.form(key='build', border=False):
                    # 创建按钮
                    st.session_state[page_name]['build_button'] = st.form_submit_button(label='Build', use_container_width=True)
        ...
        with st.container():
            ...
            with col1:
                ...
                max_iterations = st.slider("Max iterations", 0, 10,
                                           st.session_state.shared['setting']['max_iterations'])

            with col2:
                # Model selectbox
                options = ["deepseek-coder", "gpt-4o"]
                llm_model_name = st.selectbox("Model", options, options.index(st.session_state.shared['setting']['llm_model_name']))
                if llm_model_name != st.session_state.shared['setting']['llm_model_name']:
        ...
        with file_col1:
            if st.session_state[page_name]['setting']['history_list']:
                st.session_state[page_name]['setting']['history_path'] = st.selectbox("Select History File", options=st.session_state[page_name]['setting']['history_list'], key='History file')
            else:
        ...
        with file_col2:
            ...
            with st.form(key='history_form', border=False):
                # 创建按钮
                st.session_state[page_name]['reload_button'] = st.form_submit_button(label='Reload History')

        if max_iterations != st.session_state.shared['setting']['max_iterations']:
